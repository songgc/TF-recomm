...
def svd(train, test):
    ...
    with tf.Session() as sess:
        sess.run(init_op)
        print("epoch", "train_error", "val_error, elapsed_time")
        errors = deque(maxlen=samples_per_batch)
        start = time.time()
        for i in range(300 * samples_per_batch):
            ...
            if i % samples_per_batch == 0:
                ...
                print(i // samples_per_batch, train_err, np.sqrt(np.mean(test_err2)), end - start)
                start = end
